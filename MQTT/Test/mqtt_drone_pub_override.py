#!/usr/bin/env python3
"""Publish one override command to drone/{id}/cmd/override (no ROS required).

Requires: pip install paho-mqtt

Examples:
  python3 mqtt_drone_pub_override.py --payload '{"ts":0,"cmd_id":"ovr-1","action":"hold"}'
  python3 mqtt_drone_pub_override.py --payload-file override.json --drone-id drone01
  python3 mqtt_drone_pub_override.py --host 116.111.21.154 -u mqtt -P 'K4CVj6yTUx8DJMaC' \\
      --payload '{"ts":0,"cmd_id":"ovr-1","action":"hold"}'
...
def main() -> None:
    p = argparse.ArgumentParser(description="Publish MQTT drone override command")
    ...
    client_id = args.client_id or f"drone-pub-override-{int(time.time())}"
