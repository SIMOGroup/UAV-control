#!/usr/bin/env python3
"""Publish one VLA command to drone/{id}/cmd/vla (no ROS required).

Requires: pip install paho-mqtt

Examples:
  python3 mqtt_drone_pub_vla.py --payload '{"ts":0,"cmd_id":"vla-1","instruction":"..."}'
  python3 mqtt_drone_pub_vla.py --payload-file vla.json --drone-id drone01
  python3 mqtt_drone_pub_vla.py --host 116.111.21.154 -u mqtt -P 'K4CVj6yTUx8DJMaC' \\
      --payload '{"ts":0,"cmd_id":"vla-1","instruction":"hover"}'
...
def main() -> None:
    p = argparse.ArgumentParser(description="Publish MQTT drone VLA command")
    ...
    client_id = args.client_id or f"drone-pub-vla-{int(time.time())}"
